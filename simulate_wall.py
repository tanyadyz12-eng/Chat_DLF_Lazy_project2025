...
"""
simulate_wall.py
Wall-collision: interact when crossing cell walls.
"""
...
def reflect_direction(vx:int, vy:int, prefer_axis:str='auto') -> Tuple[int,int]:
    if prefer_axis == 'x':   # vertical wall
        return (-vx, vy)
    if prefer_axis == 'y':   # horizontal wall
        return (vx, -vy)
    return (-vx, -vy)

def apply_interaction(block: BlockType, beam: Laser, wall_axis: str) -> List[Laser]:
    if block == BlockType.OPAQUE:
        return []
    if block == BlockType.REFLECT:
        nvx, nvy = reflect_direction(beam.vx, beam.vy, wall_axis)
...
def pick_cell_on_wall(board: Board, x:int, y:int, vx:int, vy:int) -> Optional[Tuple[int,int,str]]:
    if x % 2 == 0 and y % 2 == 1:
        c = x // 2 if vx > 0 else x // 2 - 1
        r = (y - 1) // 2
        if 0 <= r < board.rows and 0 <= c < board.cols:
            return (r, c, 'x')
    if x % 2 == 1 and y % 2 == 0:
        r = y // 2 if vy > 0 else y // 2 - 1
        c = (x - 1) // 2
        if 0 <= r < board.rows and 0 <= c < board.cols:
            return (r, c, 'y')
    return None
...
def trace(board: Board, placements: Dict[Tuple[int,int], Block], max_steps:int=None):
    ...
    while beams:
        ...
        while steps < step_bound:
            ...
            cell_info = pick_cell_on_wall(board, nx, ny, beam.vx, beam.vy)
            if cell_info is not None:
                r, c, axis = cell_info
                blk = board.get_block(r, c, placements)
                if blk:
                    new_beams = apply_interaction(blk.kind, Laser(nx, ny, beam.vx, beam.vy), wall_axis=axis)
                    if not new_beams:
                        break
